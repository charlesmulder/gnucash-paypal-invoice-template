...
;; 02111-1307 USA
  ...
  (define (display-report opt-invoice owner endowner ownertype)
    ...
    (let* (; invoice and company details
           ...
           (customer (gncCustomerLookup book guid))
           (customer_address (gncCustomerGetAddr customer))
           (customer_name (gncCustomerGetName customer))
           (customer_id (gncCustomerGetID customer))
           (customer_notes (gncCustomerGetNotes customer))
           (customer_email (gncAddressGetEmail customer_address))
           (customer_name (gncAddressGetName customer_address))
           (customer_phone (gncAddressGetPhone customer_address))
...
<tr valign="top">
  <td align="left">
    ...
    <?scm (if coyphone (begin ?>
      <?scm:d (_ "Phone") ?>:&nbsp;<?scm:d coyphone ?><br>
    <?scm )) ?>
    <?scm (if coyfax (begin ?>
      <?scm:d (_ "Fax") ?>:&nbsp;<?scm:d coyfax ?><br>
    <?scm )) ?>
  ...
  <td align="right">
    <table border="0">
      ...
      <tr>
        <td align="right"><?scm:d (nbsp opt-invoice-number-text) ?></td>
        <td align="left"><?scm:d invoiceid ?></td>
      </tr>
      <?scm )) ?>
